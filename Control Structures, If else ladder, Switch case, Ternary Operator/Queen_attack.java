...
Problem Statement
In an 8X8 chessboard. Given the positions of the Queen as (X, Y) and 
the King as (P, Q) .
...
You just have to complete the function QueenAttack() 
that takes integers X, Y, P, and Q as arguments.
...
static int QueenAttack(int X, int Y, int P, int Q){
    if (X==P ||Y==Q || (Math.abs(X-P)==Math.abs(Y-Q))) return 1;
    return 0;
}